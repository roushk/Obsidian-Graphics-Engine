...
BoundingVolumeHeiarchy_AABB::BoundingVolumeHeiarchy_AABB(std::vector<std::pair<Object*, glm::vec3>> obj, bool topDown)
{
  if(topDown)
  {
    root.level = 0;
...
BoundingVolumeHeiarchy_AABB::~BoundingVolumeHeiarchy_AABB()
{
  deleteNodeBVHAABB(root.left);
  deleteNodeBVHAABB(root.right);
  if (root.bv != nullptr)
    delete reinterpret_cast<AxisAlignedBoundingBox*>(root.bv);
}

BoundingVolumeHeiarchyNode* BoundingVolumeHeiarchy_AABB::BuildBottomUpNodeTree(std::vector < BoundingVolumeHeiarchyNode*> parents)
{
  ...
  while(parents.size() >= 2)
  {
    ...
    parent->bv = reinterpret_cast<BoundingVolume*>(new AxisAlignedBoundingBox(objects));
...
void BoundingVolumeHeiarchy_AABB::BottomUp(BoundingVolumeHeiarchyNode* node,
  std::vector<std::pair<Object*, glm::vec3>>& obj)
{
  ...
  for(auto& object: obj)
  {
    ...
    parent->bv = reinterpret_cast<BoundingVolume*>(new AxisAlignedBoundingBox(objVec));
...
void BoundingVolumeHeiarchy_AABB::TopDown(BoundingVolumeHeiarchyNode* node,
   std::vector<std::pair<Object*, glm::vec3>>& obj)
{
  node->level++;
  ...
  if(obj.size() == 1)
  {
    node->bv = reinterpret_cast<BoundingVolume*>(new AxisAlignedBoundingBox(obj));
    node->objects = obj;
    node->type = NodeType::Leaf;
    node->bvType = BVHNodeBVType::AABB;
  }
  else if(obj.size() >= 2)
  {
    glm::vec3 min, max;
...
std::vector<Wireframe> BoundingVolumeHeiarchy_AABB::make_wireframes()
{
  std::vector<Wireframe> wireframes;
  makeWireframes(wireframes, &root);
  return wireframes;
}

void BoundingVolumeHeiarchy_AABB::makeWireframes(std::vector<Wireframe>& wireframes,BoundingVolumeHeiarchyNode* node)
{
  if (node->bv == nullptr)
    return;

  wireframes.push_back({ *reinterpret_cast<AxisAlignedBoundingBox*>(node->bv) });
  wireframes.back().color = node->color;

  if(node->left != nullptr)
  {
    makeWireframes(wireframes, node->left);
  }
  if (node->right != nullptr)
  {
    makeWireframes(wireframes, node->right);
  }
