...
void main()
{
  ...
  for( int i = 0; i < G.activeLights ; ++i )/*G.activeLights*/
  {
    ...
    //vec4 R = (2 * (NdotL) * vertexNormal) - L;  //R = 2(N.L)N - L //Were using H interpolated instead
    //vec3 V = normalize(cameraPos - vertexPosition.xyz);
    vec3 Ispecular = LA.lights[i].LightSpecular.rgb * KspecularColor * 
    pow(max(dot(vertexNormal.xyz, halfVector[i]), 0), ns); //Is Ks (R dot V)^ns
